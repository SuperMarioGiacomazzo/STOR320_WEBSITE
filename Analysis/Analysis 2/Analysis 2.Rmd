...
Suppose we want to see if countries with higher GDP are less affected by the coronavirus. Merge the data from `GDP7` into the the data frame named `COVID` to create a new dataset named `GDP7.COVID`. The dataset `COVID` is now our primary dataset.
...
In the data frame you created `GDP7.COVID`, there are some countries without GDP that are definitely in the dataset `GDP7`. Fix the country names in the dataset `GDP7` for the countries: "Democratic Republic of the Congo", "Republic of the Congo", "Sao Tome and Principe", and "Saint Kitts and Nevis". Then, redo the merge from the previous question, creating a new dataset named `GDP7.COVID.2`. Make sure the merge doesn't miss the GDP's for these specific countries, and remember that you are joining the modified data from `GDP7` into `COVID`.

```{r, eval=FALSE}
COMPLETE

head(filter(GDP7.COVID.2,Country %in% c("Democratic Republic of the Congo","Republic of the Congo", "Sao Tome and Principe","Saint Kitts and Nevis"))) #DO NOT CHANGE
```
