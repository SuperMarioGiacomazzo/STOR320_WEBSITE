...
Webscrape the data from (https://coronavirus.jhu.edu/data/mortality) into a data frame in R, select only the `Country` and `Case-Fatality` variables, and then convert the case-fatality variable to a numeric variable. Call the final dataset `COVID`.

Make the variables named `Confirmed`, `Deaths`, and `Case-Fatality` numeric by first removing the commas and percent signs and then by using the `as.numeric()` function. Also, make sure the dataset only contains the `Country` and `Case-Fatality` variables.
...
```{r,eval=FALSE}
URL.COVID=curl("https://coronavirus.jhu.edu/data/mortality") #DO NOT CHANGE

open(URL.COVID, "rb") #DO NOT CHANGE

COVID = URL.COVID %>% #DO NOT CHANGE
        COMPLETE

close(URL.COVID,"rb") #DO NOT CHANGE
